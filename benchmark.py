...
LABEL_JSON = Path('/home/william/data/stanford/semantic_labels.json')
...
ADE20K_TO_STANFORD = {
    ...
    # all the other → ignore (no beam, board, clutter, etc.)
}
...
def load_label_map(json_path: Path):
    """Load Stanford semantic_labels.json and build RGB index → class index map."""
    with open(json_path) as f:
        json_labels = json.load(f)
    labels = sorted(list(set([l.split("_")[0] for l in json_labels])))
    lbl_map = {ik: labels.index(k.split("_")[0]) 
               for ik, k in enumerate(json_labels)}
    lbl_map[int(0x0D0D0D)] = labels.index("<UNK>")
    return lbl_map, labels

# Après load_label_map
LBL_MAP, STANFORD_CLASS_NAMES = load_label_map(LABEL_JSON)

# Build LUT once globally
_max_idx = max(LBL_MAP.keys()) + 1
SEMANTIC_LUT = np.zeros(_max_idx, dtype=np.int32)
for _rgb_idx, _label_idx in LBL_MAP.items():
    if _rgb_idx < _max_idx:
        SEMANTIC_LUT[_rgb_idx] = _label_idx

def decode_semantic(semantic_path: Path) -> np.ndarray:
    """Decode Stanford semantic PNG → label index array using prebuilt LUT."""
    img = np.array(PILImage.open(semantic_path))
    indices = (img[:,:,0].astype(np.int32) * 65536 + 
               img[:,:,1].astype(np.int32) * 256 + 
               img[:,:,2].astype(np.int32))
    valid_mask = indices < len(SEMANTIC_LUT)
    semantic = np.zeros(indices.shape, dtype=np.int32)
    semantic[valid_mask] = SEMANTIC_LUT[indices[valid_mask]]
    return semantic
...
def load_stanford_sample(rgb_path: Path, gt_path: Path, task: str):
    ...
    image_bgr = cv2.resize(image_bgr, (2048, 1024))

    if task == 'segmentation':
       gt = decode_semantic(gt_path)
       gt = cv2.resize(gt.astype(np.float32), (2048, 1024), 
...
def collect_samples(data_dir: Path, task: str, n_samples: int = 50):
    ...
    """
    rgb_dir = data_dir / 'pano' / 'rgb'
    if task == 'segmentation':
        gt_dir  = data_dir / 'pano' / 'semantic'
        gt_suffix = '_domain_semantic.png'
    else:
        gt_dir  = data_dir / 'pano' / 'depth'
        gt_suffix = '_domain_depth.png'

    rgb_files = sorted(rgb_dir.glob('*_domain_rgb.png'))[:n_samples]

    samples = []
    for rgb_path in rgb_files:
        # Build corresponding GT path from RGB filename
        stem = rgb_path.stem.replace('_domain_rgb', '')
...
def run_segmentation_benchmark(data_dir: Path, output_dir: Path,
    ...
    if not samples:
        raise ValueError(f"No samples found in {data_dir}/pano/. Check paths.")
    results = []
    ...
    for model_name in SEG_MODELS:
        ...
        timing = InferenceTimer.measure(model.predict, img_rgb)
        ...
        print(f"  mIoU={row['mIoU']:.4f} | "
              ...
              f"Time={row['inference_mean_ms']}ms")
...
def _plot_segmentation_results(results: list, output_dir: Path):
    ...
    times    = [r['inference_mean_ms'] for r in results]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_dir', type=str,
                        default='/home/william/Depth-Anything-V2/checkpoints',
